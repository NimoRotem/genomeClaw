        ...
        for base_dir in [RUNS_DIR, SCRATCH_RUNS]:
            ...
            for run_dir in sorted(base_dir.iterdir(), reverse=True):
                ...
                if not rj.exists():
                    continue
                rid = run_dir.name
                # Skip if already in disk_runs
                if any(r.id == rid for r in disk_runs):
                    continue
                try:
                    results_data = json.loads(rj.read_text())
                    pgs_ids = list({r.get("pgs_id") for r in results_data if r.get("pgs_id")})
                    source_paths = list({r.get("source_path") for r in results_data if r.get("source_path")})
                    # Get file modification time as proxy for completion time
                    mtime = datetime.fromtimestamp(rj.stat().st_mtime, tz=timezone.utc)
                    disk_runs.append(RunSummary(
                        id=rid,
                        pgs_ids=pgs_ids,
                        engine="auto",
                        genome_build="GRCh38",
                    ...
                    logger.debug("Failed to read disk run %s: %s", run_dir.name, e)
        if disk_runs:
            runs = disk_runs[:limit]
        ...
        for base_dir in [RUNS_DIR, SCRATCH_RUNS]:
            ...
            if rj.exists():
                try:
                    ...
                    mtime = datetime.fromtimestamp(rj.stat().st_mtime, tz=timezone.utc)
                    source_paths = list({r.get("source_path") for r in results_data if r.get("source_path")})
                    source_files = [{"path": p, "type": "gvcf"} for p in source_paths]
                    return RunDetail(
                        id=run_id,
                        pgs_ids=pgs_ids,
                        engine="auto",
                        ...
                        started_at=mtime,
                        completed_at=mtime,
                        results_path_persistent=str(RUNS_DIR / run_id),
                        config_snapshot={"ref_population": "EUR"},
...
async def get_run_results(run_id: str, db: Session = Depends(get_db)):
    ...
    # Fallback: if DB has no results, try loading from results.json on disk
    if not flat_results:
        for base_dir in [RUNS_DIR, SCRATCH_RUNS]:
            rj = base_dir / run_id / "results.json"
            if rj.exists():
                try:
                    disk_results = json.loads(rj.read_text())
                    for i, rd in enumerate(disk_results):
                        scores = []
                        raw_score = rd.get("raw_score")
                        z_score = rd.get("z_score")
                        percentile = rd.get("percentile")
                        if raw_score is not None:
                            scores.append({
                                "sample": rd.get("sample_name"),
                                "raw_score": raw_score,
                                "z_score": z_score,
                                "pop_z_score": z_score,
                                "percentile": percentile,
                                "rank": percentile,
                                "ref_mean": rd.get("ref_mean"),
                                "ref_std": rd.get("ref_std"),
                                "ref_population": rd.get("ref_population"),
                                "variants_used": rd.get("matched_variants", 0),
                            })
                        flat_results.append(RunResultResponse(
                            id=f"disk-{i}",
                            pgs_id=rd.get("pgs_id", "?"),
                            trait=rd.get("trait"),
                            source_file_path=rd.get("source_path"),
                            source_file_type=rd.get("source_type", "gvcf"),
                            variants_matched=rd.get("matched_variants", 0),
                            variants_total=rd.get("total_variants", 0),
                            match_rate=rd.get("match_rate", 0),
                            scores_json=scores,
                        ))
                    logger.info("Loaded %d results from disk for run %s", len(flat_results), run_id)
                except Exception as e:
                    logger.error("Failed to load results.json for run %s: %s", run_id, e)
                break
