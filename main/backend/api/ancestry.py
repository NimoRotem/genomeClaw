...
from backend.config import DATA_DIR, CPU_COUNT
import logging
...
_ALIGNED_BAMS_DIR = Path(DATA_DIR) / "aligned_bams"
_NIMOG_OUTPUT_DIR = Path("/scratch/nimog_output")
...
async def list_source_files():
    """Return available source files (BAM/CRAM/VCF/gVCF) for ancestry analysis."""
    samples = []
    # BAM/CRAM files
    for pattern in [str(_ALIGNED_BAMS_DIR / "*.bam"), str(_ALIGNED_BAMS_DIR / "*.cram")]:
        for f in sorted(_glob(pattern)):
            name = os.path.basename(f).rsplit(".", 1)[0]
            ext = f.rsplit(".", 1)[-1]
            samples.append({"name": name, "path": f, "type": ext})
    # gVCF files
    for f in sorted(_glob(str(_NIMOG_OUTPUT_DIR / "*/dv/*.g.vcf.gz"))):
        name = os.path.basename(f).replace(".g.vcf.gz", "")
        samples.append({"name": name, "path": f, "type": "gvcf"})
    # VCF files
    for f in sorted(_glob(str(_NIMOG_OUTPUT_DIR / "*/dv/*.vcf.gz"))):
        if ".g.vcf.gz" in f:
            continue
        name = os.path.basename(f).replace(".vcf.gz", "")
        samples.append({"name": name, "path": f, "type": "vcf"})
    # Sort: VCF first, then gVCF, then BAM
    prio = {"vcf": 0, "gvcf": 1, "bam": 2, "cram": 3}
    samples.sort(key=lambda s: (s["name"], prio.get(s["type"], 9)))
    return samples


class RunAncestryRequest(BaseModel):
    sample_name: str
    file_path: str
    file_type: str = "bam"  # bam, cram, vcf, gvcf


# Per-sample ancestry run state
_sample_run_status: dict = {}


def _run_sample_ancestry(sample_name: str, file_path: str, file_type: str):
    """Background task: run ancestry pipeline for one sample."""
    global _sample_run_status
    _sample_run_status[sample_name] = {"status": "running", "message": f"Running ancestry for {sample_name}..."}

    script = _SCRIPTS_DIR / "run_ancestry.py"
    if not script.exists():
        _sample_run_status[sample_name] = {"status": "error", "message": f"Script not found: {script}"}
        return

    threads = str(min(CPU_COUNT, 16))
    cmd = ["python", str(script), "--sample-name", sample_name, "--threads", threads]
    if file_type in ("bam", "cram"):
        cmd.extend(["--bam", file_path])
    else:
        cmd.extend(["--vcf", file_path])

    try:
        logger.info("Starting ancestry for %s: %s", sample_name, " ".join(cmd))
        result = subprocess.run(
            cmd, capture_output=True, text=True, timeout=3600,
            cwd=str(Path(__file__).parent.parent.parent),
        )
        if result.returncode == 0:
            _sample_run_status[sample_name] = {"status": "complete", "message": "Ancestry analysis complete"}
            logger.info("Ancestry complete for %s", sample_name)
        else:
            err = result.stderr[-500:] if result.stderr else "Unknown error"
            _sample_run_status[sample_name] = {"status": "error", "message": err}
            logger.error("Ancestry failed for %s: %s", sample_name, err)
    except subprocess.TimeoutExpired:
        _sample_run_status[sample_name] = {"status": "error", "message": "Pipeline timed out (>1h)"}
    except Exception as e:
        _sample_run_status[sample_name] = {"status": "error", "message": str(e)[:500]}


@router.post("/run-sample")
async def run_sample_ancestry(body: RunAncestryRequest, background_tasks: BackgroundTasks):
    """Trigger ancestry analysis for a single sample file."""
    if not os.path.exists(body.file_path):
        raise HTTPException(400, f"File not found: {body.file_path}")
    if _sample_run_status.get(body.sample_name, {}).get("status") == "running":
        raise HTTPException(400, f"Ancestry already running for {body.sample_name}")
    background_tasks.add_task(_run_sample_ancestry, body.sample_name, body.file_path, body.file_type)
    return {"status": "started", "sample_name": body.sample_name}


@router.get("/run-sample-status/{sample_name}")
async def get_sample_run_status(sample_name: str):
    """Check status of a per-sample ancestry run."""
    return _sample_run_status.get(sample_name, {"status": "idle", "message": "No run started"})
